...
/*   Updated: 2020/02/25 12:54:39 by mrosario         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//Ray Caster function. Of course, must be split up.
int   ft_rayCaster(int key, void *param)
{
    ...
    while (x < screenWidth)
    {
       ...
       /* if (g_rayData.side == 0)
        ...
        /*g_frameData.texX = (int)(g_frameData.wallX * (double)64.0);
        if (g_rayData.side == 0 && g_rayData.rayDirX > 0)
            g_frameData.texX = (64) - (g_frameData.texX) - 1;
